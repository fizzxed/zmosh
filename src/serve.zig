...
pub const Gateway = struct {
    ...
    pub fn init(
        ...
        gw.debug_log = blk: {
            ...
            const path = std.fmt.bufPrint(&path_buf, "{s}/zmosh-server-debug.log", .{home}) catch break :blk null;
    ...
    pub fn run(self: *Gateway) !void {
        ...
        while (self.running) {
            ...
            if (now - self.last_stats_ns >= 2 * std.time.ns_per_s) {
                ...
                const pacer_can = self.pacer.canSend(now);
                const bbr_can = self.bbr.canSend();
                debugWrite(self.debug_log, "S off={d} pend={d} infl={d} cwnd={d} bw={d} s={d}.{d} lost={d} srtt={d} pacing_rate={d} pacer_can={} bbr_can={} flow={d} bp_thr={d}\n", .{
                    self.output_offset,
